...
@cython.boundscheck(False)
@cython.wraparound(False)
def construct_3d_from_circ(
    np.ndarray[np.float64_t, ndim=3] F3d,
    np.ndarray[np.float64_t, ndim=3] Fcirc,
    np.ndarray[np.float64_t, ndim=1] x_array,
    np.ndarray[np.float64_t, ndim=1] y_array,
    np.ndarray[np.int_t, ndim=1] modes,
    int nx, int ny, int nz, int nr, int nmodes,
    double inv_dr, double rmax ):

    # Define various scalars
    cdef double x
    cdef double y
    cdef complex expItheta
    cdef double cos
    cdef double sin
    ...
    for ix in xrange(nx):
        ...
        for iy in xrange(ny):
            y = y_array[iy]
            r = sqrt( x**2 + y**2 )
            ir = nr - 1 - <int> floor( (rmax - r) * inv_dr + 0.5 )
            ...
            for im in xrange(nmodes):
                ...
                if mode==0:
                    for iz in xrange(nz):
                        F3d[ix, iy, iz] += Fcirc[0, ir, iz]
                else:
                    cos = (expItheta**mode).real
                    sin = (expItheta**mode).imag
                    for iz in xrange(nz):
                        F3d[ix, iy, iz] += Fcirc[2*mode-1, ir, iz]*cos \
                                         + Fcirc[2*mode, ir, iz]*sin
